...
export const FloatingWallBotPage = withRouter(({location, history}) => {
    let {exchange, account} = useParams();
    const exchangeApi = Api.forExchangeAccount(exchange, account)
    const botApi = exchangeApi.forBot("floating-wall")
    const baseUrl = `/exchange/${exchange}/accounts/${account}/bot/floating-wall`
    ...
    return <Container fluid>
        <div id="floating-wallBot">
            <h3>[{exchange} ({account})] Floating Wall Bot</h3>
            <Tabs id={"floating-wall-bot"}
