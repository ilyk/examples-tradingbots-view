...
export const TelegramBotPage = withRouter(({location, history}) => {
    let {exchange, account} = useParams();
    const exchangeApi = Api.forExchangeAccount(exchange, account)
    const botApi = exchangeApi.forBot("telegram")
    const baseUrl = `/exchange/${exchange}/accounts/${account}/bot/telegram`
    ...
    return <Container fluid={true}>
        <div id="telegramBot">
            <h3>[{exchange} ({account})] Telegram Bot</h3>
            <Tabs id={"telegram-bot"}
