...
export class LogsPage extends React.Component {
    constructor(props) {
        ...
        this.state = {
            loaded: true,
            from: new Date(localStorage.getItem("hunter-log-date-from") || new Date() - (60 * 60 * 1000)),
            to: localStorage.getItem("hunter-log-date-to") ? new Date(localStorage.getItem("hunter-log-date-to")) : new Date(),
            severity: localStorage.getItem("hunter-log-severity") || priority.Info
        }
    }
            ...
            >
                <Container fluid className={"mt-3"} style={{height: "500px"}}>
                    <LogViewer
                        ...
                        lsPrefix={"hunter-"}
                        tag={`${this.props.account}-${text.capitalize(this.props.exchange)}HunterExchangeBot`}
                    />
                </Container>
            </LoadingOverlay>
        );
