...
export const IcebergBotPage = withRouter(({location, history}) => {
    let {exchange, account} = useParams();
    const exchangeApi = Api.forExchangeAccount(exchange, account)
    const botApi = exchangeApi.forBot("iceberg")
    const baseUrl = `/exchange/${exchange}/accounts/${account}/bot/iceberg`
    ...
    return <Container fluid>
        <div id="icebergBot">
            <h3>[{exchange} ({account})] Iceberg Bot</h3>
            <Tabs id={"iceberg-bot"}
