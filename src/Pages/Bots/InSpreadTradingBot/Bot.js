...
export const InSpreadTradingBotPage = withRouter(({location, history}) => {
    let {exchange, account} = useParams();
    const exchangeApi = Api.forExchangeAccount(exchange, account)
    const botApi = exchangeApi.forBot("inspread")
    const baseUrl = `/exchange/${exchange}/accounts/${account}/bot/inspread`
    ...
    return <Container fluid>
        <div id="inspreadBot">
            <h3>[{exchange} ({account})] InSpread Trading Bot</h3>
            <Tabs id={"inspread-bot"}
